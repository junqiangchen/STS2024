...
class UpTransition3d(nn.Module):
    def __init__(self, inChans, outChans, nConvs, prob=0.2):
        ...
        self.up_conv = nn.Upsample(scale_factor=(2, 2, 2), mode='trilinear')
    ...
    def forward(self, x, skipx):
        x = self.relu(self.drop(self.bn(self.conv(x))))
        out = self.up_conv(x)
        xcat = torch.add(out, skipx)
        out = self.ops(xcat)
...
class VNet3dthin(nn.Module):
    """
    VNet3dthin implement
    """

    # the number of convolutions in each layer corresponds
    # to what is in the actual prototxt, not the intent
    def __init__(self, image_channel, numclass, init_features=16):
        super(VNet3dthin, self).__init__()
        ...
        self.up_tr64 = UpTransition3d(self.features * 4, self.features * 2, 3, prob=0.2)
        self.up_tr32 = UpTransition3d(self.features * 2, self.features, 2, prob=0.2)
