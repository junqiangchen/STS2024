...
def save_images3d(pdmask: Tensor, gtmask: Tensor, size, path, pixelvalue=255.):
    ...
    cv2.imwrite(path + "pdmask.png", np.clip(merge_img * pixelvalue, 0, 255).astype('uint8'))
    ...
    cv2.imwrite(path + "gtmask.png", np.clip(merge_mask * pixelvalue, 0, 255).astype('uint8'))
...
def save_images3dregression(src: Tensor, pdmask: Tensor, gtmask: Tensor, size, path, pixelvalue=255.):
    images = pdmask.detach().cpu().squeeze().numpy()
    gtmask = gtmask.detach().cpu().squeeze().numpy()
    ...
    merge_src = np.zeros((h * size[0], w * size[1]))
    for idx, image in enumerate(src):
        i = idx % size[1]
        j = idx // size[1]
        merge_src[j * h:j * h + h, i * w:i * w + w] = image
    cv2.imwrite(path + "src.png", np.clip(merge_src * pixelvalue, 0, 255).astype('uint8'))
