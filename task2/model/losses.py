...
class L2Loss(nn.Module):
    ...
    def forward(self, y_pred_logits, y_true):
        bs = y_pred_logits.size(0)
        num_classes = y_pred_logits.size(1)
        y_pred_logits = y_pred_logits.float().contiguous().view(bs, num_classes, -1)
        y_true = y_true.float().contiguous().view(bs, num_classes, -1)
        l2 = torch.sum((y_true - y_pred_logits) ** 2)
        return l2
...
class SSIMLoss(nn.Module):
    ...
    def __init__(self):
        ...
        self.ssimloss = SSIM3D()

    def forward(self, y_pred_logits, y_true):
        (batchsize, channel, Z, W, H) = y_pred_logits.size()
        y_pred_logits = y_pred_logits.view(batchsize, channel, Z, W, H).float()
        y_true = y_true.view(batchsize, channel, Z, W, H).float()
        ssimsum = 0
        for batch in range(batchsize):
            y_pred = y_pred_logits[batch]
            y_gt = y_true[batch]
            y_pred = y_pred.view(1, channel, Z, W, H).float()
            y_gt = y_gt.view(1, channel, Z, W, H).float()
            ssimsum = ssimsum + self.ssimloss(y_pred, y_gt)
        return 1 - ssimsum / batchsize
...
class L1SSIMLoss(nn.Module):
    ...
    def forward(self, y_pred_logits, y_true):
        bs = y_true.size(0)
        num_classes = y_pred_logits.size(1)
        y_pred_logits1 = y_pred_logits.float().contiguous().view(bs, num_classes, -1)
        y_true1 = y_true.float().contiguous().view(bs, num_classes, -1)
        l1 = self.l1loss(y_pred_logits1, y_true1)
        ssim = self.ssimloss(y_pred_logits, y_true)
        ssim = 1 - ssim
        return l1 + ssim
