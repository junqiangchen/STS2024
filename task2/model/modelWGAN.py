...
class WGAN2dModel(object):
    ...
    def __init__(self, image_height, image_width, image_channel, numclass, batch_size, inference=False,
                 model_path=None, amp=True, num_cpu=8, use_cuda=True):
        ...
        for batch_idx, batch in enumerate(train_loader):
            ...
            if self.amp:
                with autocast():
                    ...
                    # Gradient penalty
                    gradient_penalty = self._compute_gradient_penalty(self.discriminator, y.data, fake_imgs.data)
                    d_loss, g_loss = self._loss_function(real_validity, fake_validity, gradient_penalty)
                # first, zero out any previously accumulated gradients,
                # then perform backpropagation,
                # and then update model parameters
            ...
            psnr, ssim = self._accuracy_function(self.accuracyname, fake_imgs, y, mean, std)
            if trainshow:
                # save_images
    ...
    def trainprocess(self, trainimage, trainmask, validationimage, validationmask, model_dir, epochs=50, lr=1e-4):
        print("[INFO] training the network...")
        Path(model_dir).mkdir(parents=True, exist_ok=True)
        summary(self.generator, input_size=(1, 64, 64))
        print(self.generator)
        summary(self.discriminator, input_size=(1, 64, 64))
        print(self.discriminator)
        ...
        self.discriminator.apply(initialize_weights)
        # Optimizers
        self.optimizer_G = torch.optim.Adam(self.generator.parameters(), lr=lr, betas=(0.5, 0.9))
        self.optimizer_D = torch.optim.Adam(self.discriminator.parameters(), lr=lr, betas=(0.5, 0.9))
