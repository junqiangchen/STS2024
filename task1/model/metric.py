...
def calc_abs(input: Tensor, target: Tensor):
    num = target.size(0)
    input = input.view(num, -1).float()
    target = target.view(num, -1).float()
    mse = abs(input - target)
    return torch.mean(mse)
...
def calc_psnrv2(input: Tensor, target: Tensor):
    """Peak Signal to Noise Ratio
    PSNR = 20 * log10(MAXp) - 10 * log10(MSE)"""
    num = target.size(0)
    input = input.view(num, -1).float()
...
def calc_ssimv2(input: Tensor, target: Tensor):
    """ssim"""
    num = target.size(0)
    input = input.view(num, -1).float()
    target = target.view(num, -1).float()
    ...
    ssim = 0
    for batch in range(num):
        real_image = (target[batch]) / (torch.max(target[batch]) + eps)
        pred_image = (input[batch]) / (torch.max(input[batch]) + eps)
        ssim += compare_ssim(real_image.detach().cpu().squeeze().numpy().astype(np.float16),
                             pred_image.detach().cpu().squeeze().numpy().astype(np.float16))
    return ssim / num
