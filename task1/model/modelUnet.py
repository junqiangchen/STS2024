...
from networks.Unet3dthin import UNet3dthin
from networks import initialize_weights
from .dataset import datasetModelSegwithopencv, datasetModelSegwithnpy, datasetModelSegwithnpymutil, \
    datasetModelRegressionwithnpy, datasetModelRegressionwithopencv
from torch.utils.data import DataLoader
from .losses import BinaryDiceLoss, BinaryFocalLoss, BinaryCrossEntropyLoss, BinaryCrossEntropyDiceLoss, \
    MutilDiceLoss, MutilFocalLoss, MutilCrossEntropyLoss, MutilCrossEntropyDiceLoss, MSELoss, L1Loss
import torch.optim as optim
from torch.cuda.amp import autocast as autocast
from torch.cuda.amp import GradScaler
import numpy as np
from tqdm import tqdm
from .metric import dice_coeff, iou_coeff, multiclass_dice_coeff, multiclass_iou_coeff, calc_psnr, calc_mse, calc_ssim
...
class BinaryUNet2dModel(object):
    ...
    def __init__(self, image_height, image_width, image_channel, numclass, batch_size, loss_name='BinaryDiceLoss',
                 inference=False, model_path=None, amp=True, accum_gradient_iter=1, num_cpu=8, use_cuda=True):
...
class MutilUNet2dModel(object):
    ...
    def _train_loop(self, train_loader, totalTrainLoss, totalTrainAccu, trainshow, e):
        for batch_idx, batch in enumerate(train_loader):
            ...
            if trainshow:
                savepath = self.model_dir + '/' + str(e + 1) + "_Train_EPOCH_"
                save_images2d(torch.argmax(pred[0], 0), y[0], savepath, pixelvalue=self.showpixelvalue)
                trainshow = False
            # add the loss to the total training loss so far
            totalTrainLoss.append(loss.cpu().detach().numpy() * self.accum_iter)
            totalTrainAccu.append(accu.cpu().detach().numpy())
...
class BinaryUNet3dModel(object):
    ...
    def _dataloder(self, images, labels, batch_size, shuffle=False):
        ...
        train dataset shuffle is true,validation is false
        """
        dataset = datasetModelSegwithnpymutil(images, labels,
                                              targetsize=(
    ...
    def inference(self, imagesitk, imagesitk2, newSize=(96, 96, 96)):
        ...
        _, resizeimagesitk2 = resize_image_itkwithsize(imagesitk2, newSize, imagesitk2.GetSize(),
        ...
        imageresize2 = sitk.GetArrayFromImage(resizeimagesitk2)
        imageresize2[imageresize2 != 0] = 1
        # transpose (D,H,W,C) order to (C,D,H,W) order
        D, H, W = np.shape(imageresize)[0], np.shape(imageresize)[1], np.shape(imageresize)[2]
        image = np.zeros((self.image_channel, D, H, W))
        image[0, ...] = imageresize[:]
        image[1, ...] = imageresize2[:]
...
class BinaryUNet3dthinModel(object):
    """
    Unet3dthin with binary class,should rewrite the dataset class and inference fucntion
    """
    ...
    def __init__(self, image_depth, image_height, image_width, image_channel, numclass, batch_size,
        ...
        self.model = UNet3dthin(self.image_channel, self.numclass)
        self.model.to(device=self.device)
...
class MutilUNet3dModel(object):
    ...
    def _train_loop(self, train_loader, totalTrainLoss, totalTrainAccu, trainshow, e):
        for batch_idx, batch in enumerate(train_loader):
            ...
            if trainshow:
                savepath = self.model_dir + '/' + str(e + 1) + "_Train_EPOCH_"
                save_images3d(torch.argmax(pred[0], 0), y[0], self.showwind, savepath, pixelvalue=self.showpixelvalue)
                trainshow = False
            # add the loss to the total training loss so far
            totalTrainLoss.append(loss.cpu().detach().numpy() * self.accum_iter)
            totalTrainAccu.append(accu.cpu().detach().numpy())
        ...
        with torch.no_grad():
            ...
            for batch_idx, batch in enumerate(val_loader):
                ...
                if trainshow:
                    # save_images
                    savepath = self.model_dir + '/' + str(e + 1) + "_Val_EPOCH_"
                    save_images3d(torch.argmax(pred[0], 0), y[0], self.showwind, savepath,
...
class UNet2dRegressionModel(object):
        ...
        for batch_idx, batch in enumerate(train_loader):
            ...
            if trainshow:
                ...
                save_images2dregression(x[0] * std[0] + mean[0],
                                        ...
                                        y[0] * std[0] + mean[0],
                                        savepath, pixelvalue=self.showpixelvalue)
                trainshow = False
            # add the loss to the total training loss so far
            totalTrainLoss.append(loss.cpu().detach().numpy() * self.accum_iter)
    ...
    def trainprocess(self, trainimage, trainmask, validationimage, validationmask, model_dir, epochs=50, lr=1e-3):
        ...
        for e in tqdm(range(epochs)):
            ...
            # 4.8、save best_validation_dsc model params
            # serialize best model to disk
            MODEL_PATH = os.path.join(self.model_dir, "unet_%d.pth" % e)
            torch.save(self.model.state_dict(), MODEL_PATH)
            # 4.9、clear cache memory
            self.clear_GPU_cache()
        # display the total time needed to perform the training
...
class UNet3dRegressionModel(object):
    ...
    def __init__(self, image_depth, image_height, image_width, image_channel, numclass, batch_size,
                 loss_name='MSE', inference=False, model_path=None, amp=True, accum_iter=1, num_cpu=8, use_cuda=True):
        self.batch_size = batch_size
        self.loss_name = loss_name
        self.accuracyname = ['PSNR', 'SSIM']
        self.image_height = image_height
